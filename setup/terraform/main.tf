...
resource "aws_subnet" "public_subnet" {
  ...
  availability_zone       = "ap-south-2${var.public_az}"
...
resource "aws_subnet" "private_subnet" {
  vpc_id            = aws_vpc.vpc.id
  availability_zone = "ap-south-2${var.private_az}"
...
resource "aws_vpc_endpoint" "eks" {
  ...
  vpc_id              = aws_vpc.vpc.id
  service_name        = "com.amazonaws.ap-south-2.eks"
...
resource "aws_vpc_endpoint" "ec2" {
  ...
  vpc_id              = aws_vpc.vpc.id
  service_name        = "com.amazonaws.ap-south-2.ec2"
  vpc_endpoint_type   = "Interface"
  security_group_ids  = [aws_eks_cluster.main.vpc_config.0.cluster_security_group_id]
  private_dns_enabled = true
}
...
resource "aws_codebuild_project" "codebuild" {
  ...
  source {
    type            = "GITHUB"
    location        = "https://github.com/chaitanyamouli/movie-picture-pipeline.git"
    git_clone_depth = 1
